...
const NavigationBar = () => {
  ...
  const [localStorageIsSuperuser, setLocalStorageIsSuperuser] = useState<
    boolean | null
  >(null);
  ...
  const getMembers = async () => {
    ...
    if (localStorageId) {
      const currentUser = fetchedMembers.find(
        (member: MemberType) => member.id === localStorageId
      );
      setCurrentUser(currentUser);

      const otherMembers = fetchedMembers.filter(
        (member: MemberType) => member.id !== localStorageId
      );
              ...
              <ListItem key={bookListType.id} disablePadding>
                ...
                <ListItemButton onClick={() => handleGenreClick(bookListType)}>
                  ...
                  <ListItemText
                    ...
                    style={{
                      color:
                        bookListType.id === lastSegment ? "red" : "inherit",
                    }}
                  />
                </ListItemButton>
              </ListItem>
              ...
              <>
                ...
                {currentUser && (
                  <ListItem key={currentUser.id} disablePadding>
                    <ListItemButton
                      onClick={() => handleMemberClick(currentUser)}
                    >
                      <ListItemIcon>
                        <UserRound
                          className="mr-2"
                      ...
                      <ListItemText
                        ...
                        style={{
                          color:
                            currentUser.id === lastSegment ? "red" : "inherit",
                        }}
                      />
                    </ListItemButton>
                  </ListItem>
                  ...
                  <ListItem key={member.id} disablePadding>
                    <ListItemButton onClick={() => handleMemberClick(member)}>
                      ...
                      <ListItemText
                        ...
                        style={{
                          color: member.id === lastSegment ? "red" : "inherit",
                        }}
                      />
                    </ListItemButton>
                  </ListItem>
